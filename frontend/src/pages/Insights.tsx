...
export default function Insights() {
  ...
  const loadAIInsights = async (sessionId: string) => {
    ...
    try {
      ...
      const convRes = await fetch(`${API_URL}/api/conversations/${sessionId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
      ...
      const res = await fetch(`${API_URL}/api/conversations/ai-insights/${sessionId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
        ...
        <div className="flex gap-2 mb-6">
          {(['ai-insights', 'voice-stress', 'trends', 'journal'] as const).map(tab => (
            <button
              ...
              className={`pixel-button px-4 py-2 capitalize text-sm font-medium pixel-text transition-all ${activeTab === tab
              ...
              {conversations.length === 0 ? (
                ...
                <div className="grid md:grid-cols-2 gap-2">
                  {conversations.slice(0, 6).map((conv) => (
                    <div
                      key={conv.sessionId}
                      className={`relative p-3 rounded-lg border-2 transition-all ${selectedSession === conv.sessionId
                ...
                {selectedConversation?.intakeSummary && (
                  <AnimatedCard className="p-6 bg-gradient-to-br from-indigo-50 to-purple-50">
                    ...
                    <div className="flex gap-3 flex-wrap">
                      <span className={`px-4 py-2 rounded-lg font-semibold ${selectedConversation.riskLevel === 'high' ? 'bg-red-100 text-red-700 border-2 border-red-300' :
                    ...
                    <div className="space-y-2">
                      {conversations.slice(0, 10).map((conv, i) => (
                        <div key={conv.sessionId} className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 hover:border-purple-300 transition-colors">
                          ...
                          <div className={`flex-1 h-2 rounded-full ${conv.riskLevel === 'high' ? 'bg-red-400' :
